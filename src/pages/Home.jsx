       ...
       >
          {menus?.map((menu, i) => (
            ...
            <Link
              to={menu?.link}
              key={i}
              className={`grid grid-cols-3 active:bg-darkGrey  outline-none  hover:bg-darkGrey ${open?"md:hover:bg-darkGrey":""}  px-[20px]  rounded-md ml-[-10px]  ${show?"sm:ml-[-30px]":"sm:ml-[30px]"}  `}
              onClick={() => setopen(!open)}>
                <div className="">
                <img
                src={menu?.icon}
              ...
              <h2
                ...
                } font-St text-[14px] ml-[-20px] overflow-hidden md:ml-0 sm:ml-[-20%]  md:text-center ${show? "sm:text-left":""} text-left w-[100px]`}>
                {menu?.name}
              </h2>
        ...
        <div>
          <div className={`sm:${show ? "hidden" : "block"} `}>
            <AiFillSetting
              size={25}
